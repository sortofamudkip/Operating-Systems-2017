...
//Faces Difficulties: the whole strcuture is very complex, you need to check the actual meaning of every layer.
// IF you want to use the effect of revised edition, you should rebuild kernel.
// list_first_entry function's parameter is hard to know what it actually meaning.
...
static struct task_struct *pick_next_task_weighted_rr(struct rq *rq)
{
	...
	else next = list_first_entry(&rq->weighted_rr.queue, struct task_struct, weighted_rr_list_item);
	return next;
	// ...
}
